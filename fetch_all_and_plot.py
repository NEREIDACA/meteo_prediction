...
# You will need to include the logic for:
# - fetch_data_for_location(lat, lon)
# - fetch_data_soil_for_location(lat, lon)
# - arrange_predictions(df)
# - save_plot_as_html(df, predictions, members, output_path)
# This script loops over catchment points and saves forecast plots as HTMLs.

# fetch_all_and_plot.py
#globals().clear()
...
def save_plot_as_html(df, predictions, members, outpath):
    ...
    # Update layout
    fig.update_layout(
        title=f"Predicció meteorològic",
...
def fetch_all_points_data():
    results = {}
    for basin, points in catchment_points.items():
        for point in points:
            lat = point["lat"]
            lon = point["lon"]
            name = point["name"]
            location_id = f"{basin}_{name.replace(' ', '_')}"

            print(f"Fetching data for {basin} - {name}...")
            df = fetch_data(lat, lon)
            df_soil = fetch_data_soil(lat, lon)
            predictions = arrange_predictions(df_soil)
            plot_data(df, predictions)
            save_plot_as_html(df, predictions, members, f"static/{location_id}.html")
            
            results[f"{basin}_{name}"] = {
                "df": df,
                "soil": df_soil,
...
# Run the fetch and plot
fetch_all_points_data()
